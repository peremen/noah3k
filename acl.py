...
class acl:
    """
    Noah3K ACL 클래스. Noah3K 보안 설정의 가장 핵심을 이룬다.
    ...
    def is_allowed(session_key, object_type, object_id, permission):
        """
        지정한 개체에 세션 키로 지정한 사용자가 권한을 가지고 있는지 알려 준다.
        세션 키가 없으면 비 로그인 사용자로 취급한다.
